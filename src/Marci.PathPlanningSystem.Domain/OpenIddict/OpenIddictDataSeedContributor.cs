...
namespace Marci.PathPlanningSystem.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("PathPlanningSystem") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "PathPlanningSystem", DisplayName = "PathPlanningSystem API", Resources = { "PathPlanningSystem" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["PathPlanningSystem_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["PathPlanningSystem_Web:RootUrl"]!.EnsureEndsWith('/');

            /* PathPlanningSystem_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["PathPlanningSystem_Web:ClientSecret"] ?? "1q2w3e*",
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["PathPlanningSystem_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["PathPlanningSystem_Swagger:RootUrl"]?.TrimEnd('/');
